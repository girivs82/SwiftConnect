...
func launch(samlv2: Bool, ext_browser: Bool, openconnect_path: String, proto: String, gateway: String, intf: String, session_token: String, server_cert_hash: String, username: String, password: String) {
    // Prepare commands
    Logger.openconnect.info("start")
    if samlv2 {
        // External browser invocation
        if ext_browser {
            ProcessManager.shared.launch(tool: URL(fileURLWithPath: openconnect_path), arguments: ["--protocol=\(proto)", "--interface=\(intf)", "\(gateway)"]) { status, output in
                Logger.openconnect.info("completed")
            }
        }
        // SAMLv2 using embedded webkit based session
        else {
            ProcessManager.shared.launch(tool: URL(fileURLWithPath: openconnect_path), arguments: [ "--protocol=\(proto)", "--interface=\(intf)", "--cookie-on-stdin", "--servercert=\(server_cert_hash)", "\(gateway)"], input: Data("\(session_token)\n".utf8)) { status, output in
                Logger.openconnect.info("completed")
            }
        }
    }
    // Non SAML based auth
        ...
        ProcessManager.shared.launch(tool: URL(fileURLWithPath: openconnect_path), arguments: ["--protocol=\(proto)", "--interface=\(intf)", "-u", "\(username)", "--passwd-on-stdin", "\(gateway)"], input: Data("\(password)\n".utf8)) { status, output in
                Logger.openconnect.info("completed")
            }
    }
    ...
    xpc_connection_set_event_handler(peer) { request in
        if xpc_get_type(request) == XPC_TYPE_DICTIONARY {
            ...
            if cmd == "connect" {
                ...
                intf = String(cString: xpc_dictionary_get_string(request, "vpnInterface")!)
                ...
                DispatchQueue.main.async {
                    launch(samlv2: samlv2, ext_browser: ext_browser, openconnect_path: openconnect_path, proto: proto, gateway: gateway, intf: intf, session_token: session_token, server_cert_hash: server_cert_hash, username: username, password: password)
                }
            }
